...
- `제외 모드` 체크박스를 선택하면 제외 영역(반투명 **검은색** 오버레이)을 그릴 수 있습니다. `포함/제외 전환` 버튼으로 선택된 도형을 포함과 제외 사이에서 전환할 수 있습니다. 포함과 제외 영역이 겹칠 경우 제외가 우선 적용됩니다 — `Paint Selected Region`은 제외 영역을 뺀 나머지 부분에만 새 레이어를 생성합니다.
...
- **영역 페인트 제외 모드** — "제외 모드" 체크박스를 추가하여 제외 영역(검은색 오버레이)을 포함 영역(빨간색 오버레이)과 함께 그릴 수 있습니다. 겹치는 영역에서는 제외가 우선됩니다. "포함/제외 전환" 버튼으로 선택된 도형을 전환할 수 있습니다.
- **영역 페인트 총 예산 수정** — 총 예산 입력란이 이제 올바르게 작동합니다. 이전에는 품질 프로필 선택 후 값을 변경해도 효과가 없었으며, 예산이 항상 프로필의 `stopAt`에서 읽혔습니다. 이제 사용자의 입력 값이 모든 예산 확인 및 남은 예산 표시에 올바르게 사용됩니다.
...
- **Text Vinyl** — 유니코드 텍스트로 FH6 타입코드 JSON을 생성하는 새로운 Text 탭. 라틴, 일본어, 한국어, 중국어(간체/번체) 지원 및 현지화된 UI.
  - 텍스트를 입력하거나 붙여넣어 비닐 레이어 생성, 글꼴 파일 업로드 또는 시스템에 설치된 CJK 글꼴 검색 (알파벳순 정렬).
  - DxBang 색상 변환기 통합으로 16진수 값 기반의 정확한 텍스트 색상 선택.
  - 기존 가져오기 흐름에 연결, 원형 템플릿 호환성 안내 포함.
  - 픽셀 아트 이미지를 타입코드 지오메트리로 변환하는 추적기.
  - 여러 탭에서 색상을 재사용할 수 있는 공유 색상 견본 시스템.
